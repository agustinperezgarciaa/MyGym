(function () {
  ...
  function SocialAccountsController($state, $window, UsersService, Authentication, Notification) {
    ...
    function onRemoveSocialAccountSuccess(response) {
      ...
      Notification.success({ message: '<i class="glyphicon glyphicon-ok"></i> Removido correctamente!' });
      vm.user = Authentication.user = response;
    }

    function onRemoveSocialAccountError(response) {
      Notification.error({ message: response.message, title: '<i class="glyphicon glyphicon-remove"></i> Error al remover!' });
    }
